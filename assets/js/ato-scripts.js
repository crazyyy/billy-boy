...
jQuery(document).ready(function() {
  ...
  jQuery('a').filter(':not(.fancybox-text)').filter(function() {
    ...
    jQuery('html, body').animate({
      scrollTop: jQuery(target.attr("href")).offset().top - 140
    });
  });
...
jQuery(window).load(function() {
  ...
  jQuery('.product-image-slider').on('afterChange', function() {
    updateElevateZoom();
  });
  updateElevateZoom();
});
 ...
 * @author Andreas Nieswand
 */
bb.Navigation = function(options) {
  this.init(options);
};
        ...
        try {
          this.searchAutocompleteCall = jQuery.ajax({
            ...
            success: function(data) {
              if (JSON.stringify(data.products) != '[]' || JSON.stringify(data.content) != '[]') {
